...
// https://editor.p5js.org/kfahn/sketches/-j45Nn1cb
...
new p5((sa) => {
  ...
  sa.preload = () => {
    data = sa.loadJSON("rules.json");
  }

  sa.setup = () => {
    canvas = sa.createCanvas(600, 600);
    canvas.position(250, 200);
    // p5grain.setup();

    controls = sa.addLsystem("Gear", "dragon2");
  }

  sa.handleInput = (sliders, dropdowns, resetButton) => {
  ...
  sa.addLsystem = (currentShape, currentRule) => {
  ...
  sa.addMessages = (newMessage, warning, addWarning) => {
    let addMessage = true;
    let message = null;
    if (newMessage != null) {
      message = newMessage;
    ...
    shapeMessage = sa.createP(message);
    ...
    ruleWarning = sa.createP(warning);
