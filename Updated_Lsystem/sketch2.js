...
function reset() {
  ...
  let currentFractal = ruleDropdown0.value()
  pickRule(currentFractal);
  addWarning(sliders0, ruleDropdown0, shapeDropdown0);
  pop();
  if (selectedShape === "word") {
    addText();
  }
   ...
   rotate(angle);
   // pickRule() must be after rotate(angle) for rotation to work properly
   currentFractal = ruleDropdown1.value();
   pickRule(currentFractal);
   addWarning(sliders0, ruleDropdown1, shapeDropdown1);
   pop();
   if (selectedShape === "word") {
     addText();
   }
...
function addWarning(sliders0, ruleDropdown, shapeDropdown) {
  // pickRule(currentFractal);
  let level = sliders0[2].value();
  if (sliders0[2].value() > 1 && ruleDropdown.value() === "circular") {
    stroke(255);
    text(
