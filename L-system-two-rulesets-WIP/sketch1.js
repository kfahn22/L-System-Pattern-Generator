...
function setSystemVariables(lsystems) {
  // Add arrays to hold the data
   let lsystemValues0 = [];
   let lsystemValues1 = [];
   let lsystemValues = [lsystemValues0, lsystemValues1]

  for (let i = 0; i < 1; i++) {
    let controls = lsystems[i][0];
    let values = updateValues(lsystems[i]);

    // lsystemValues[0] = lsystems[i][0]; // AddControls
    // lsystemValues[1] = updateValues(lsystems[i]);
    ...
    // lsystemValues[2] = lsystemValues[0].ruleset; // Ruleset object
    // lsystemValues[3] = lsystemValues[0].shape_ui; // Shape_ui object
    ...
    lsystemValues[i][0] = values;
    lsystemValues[i][1] = shape_ui;
    lsystemValues[i][2] = ruleset;
    lsystemValues[i][3] = currentStrokePalette;
    lsystemValues[i][4] = currentFillPalette;
    lsystemValues[i][5] = colorMode;
    lsystemValues[i][6] = images;
    //Add turtle system
    //console.log(lsystemValues[0][0])
    //  console.log(currentFillPalette)
    //  console.log(colorMode)
    let turtle = new Turtle(
      // lsystemValues[1], // data
      // lsystemValues[3], // shape_ui
      // lsystemValues[2], // ruleset
      // currentStrokePalette,
      // currentFillPalette,
      // lsystemValues[4], // colorMode
      // images
    lsystemValues[i][0], // values;
    lsystemValues[i][1], // shape_ui;
    lsystemValues[i][2], //ruleset;
    ...
    //  if (lsystemValues[4] == 0 || lsystemValues[4] == 1) {
    //    // Pass value of colorMode to turtle to indicate whether stroke or fill should be used to render Lsystem
    //    turtle.addLsystem(lsystemValues[4]);
    //  } else if (lsystemValues[4] == 2) {
    //    // If both stroke and fill are checked, render the L-system twice, resetting the sentence between renders. If both are rendered at same time stroke shows through the fill (because looks best with alpha < 255) and the pattern is not as nice
    //    turtle.addLsystemStrokeFill();
    //  }
    // if (lsystemValues[7] == true) {
    ...
    addMessages(
      ...
      turtle.addWarning
      // lsystemValues[5].warning, // turtle
      // lsystemValues[5].addWarning
    );
  }

  // // Add messages for shape parameters and rule level
  // addMessages(
  //   lsystemValues[3].message, // shape_ui
  //   lsystemValues[5].warning, // turtle
  //   lsystemValues[5].addWarning
  // );
}
