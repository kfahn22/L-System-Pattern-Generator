...
function setup() {
  ...
  lsystem0 = addLsystemDropdowns(x, "ADH231a", "astroid", "blue", "purple");
...
function addLsystemDropdowns(x, lsystem, shape, fillColor, strokeColor) {
  let ruleDropdown = addRuleDropdown(x, 5, lsystem);
  let shapeDropdown = addShapesDropdown(x, 35, shape);
  strokeDropdown = addPalettes(x, 80, strokeColor);
  strokeLabel = createP("Stroke color");
  strokeLabel.position(x, 45);
  let fillDropdown = addPalettes(x, 120, fillColor);
...
function addFractal(
  ...
  fillPalette = createPaletteFromURL(fillURL);
...
function addControls(pos) {
  ...
  backgroundDropdown = addPalettes(pos - 60, 60, "orange");
